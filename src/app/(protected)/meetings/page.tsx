...
const MeetingsPage = () => {
    ...
    return (
        <>
            ...
            {meetings && meetings.length === 0 && <div>No meetings foud</div>}
            {isLoading && <div>Loading...</div>}
            <ul className='divide-y divide-gray-200'>
                {meetings?.map(meeting => (
                    <li key={meeting.id} className='py-5 flex justify-between items-center gap-x-6'>
                        <div className=''>
                        ...
                        <div className='flex items-center flex-none gap-s-4'>
                            <Link href={`/meetings/${meeting.id}`}>
                                <Button variant={'outline'} size='sm' className='mr-2'>View Meeting</Button>
                            </Link>
